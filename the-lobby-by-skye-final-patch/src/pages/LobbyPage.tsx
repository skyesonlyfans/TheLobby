...
import { Phone, Users, Hash, Send, Video, Plus, X, Link as LinkIcon, Mic, UserPlus, MessageCircle } from 'lucide-react'
...
export default function LobbyPage() {
  const { user, profile, ready, updateMyProfile } = useAuth()
...
function claimUniqueUsername(room: ReturnType<typeof joinRoom>, desired: string) {
  if (!me || !profile) return desired
  const map = room.usernames
  const clean = (desired || '').trim().replace(/\s+/g, ' ').slice(0, 20) || 'SkyeUser'
  const current = map.get(clean)
  if (!current || current === me.uid) {
    map.set(clean, me.uid)
    if (profile.username !== clean) updateMyProfile({ ...profile, username: clean })
    return clean
  }
  // Taken: append short suffix until free
  for (let i = 2; i < 999; i++) {
    const candidate = `${clean}${i}`
    const v = map.get(candidate)
    if (!v || v === me.uid) {
      map.set(candidate, me.uid)
      updateMyProfile({ ...profile, username: candidate })
      return candidate
    }
  }
  const fallback = `${clean}-${me.uid.slice(0, 4)}`
  map.set(fallback, me.uid)
  updateMyProfile({ ...profile, username: fallback })
  return fallback
}
  ...
  useEffect(() => {
    ...
    // claim a unique username inside this room
    const claimed = claimUniqueUsername(room, me.username)
    awareness.setLocalStateField('user', { ...me, username: claimed })
                ...
                >
                  ...
                  {p.uid !== me?.uid ? (
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" onClick={() => nav(`/dm/${p.uid}`)} title="DM">
